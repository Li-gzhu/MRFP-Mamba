...
class SimpleRetention(nn.Module):
    ...
    def _get_D(self, sequence_length):
        ...
        n = torch.arange(sequence_length).unsqueeze(1).to(device)
        m = torch.arange(sequence_length).unsqueeze(0).to(device)
...
class MultiScaleRetention(nn.Module):
    def __init__(self, hidden_size, heads, double_v_dim=False):
        ...
        self.swish = lambda x: x * torch.tanh(x)
        # self.swish = lambda x: x * torch.softmax(x,dim=1)
    ...
    #
    # def forward_chunkwise(self, x_i, r_i_1s, i):#train
    #     """
