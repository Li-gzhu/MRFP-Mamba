...
def get_model(name, **kwargs):
    ...
    n_classes = kwargs["n_classes"]#kwargs它允许将一系列的关键字参数以字典的形式传递给函数。
    ...
    #     kwargs.setdefault("batch_size", 64)
        ...
        center_pixel = True
        model = gaht.gaht(img_size=11,#(img_size=patch_size)
            n_groups=[2,2,2], depths=[1,2,1], num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 11)
        center_pixel = True
        model = gaht_fhr.gaht(img_size=11,#(img_size=patch_size)
            n_groups=[2,2,2], depths=[1,2,1], num_heads=[8, 4, 2], num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
...
#   from   models_hit
        ...
        embed_dims = [480, 480, 516, 516]  ## for IN 400, for GRSS 256, for PU 168, for KSC 320 for XA 480
...
class HybridEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=15):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(4224, 64, (3, 3), padding=0, stride=(1, 1)),
