...
class CNNEncoder2(nn.Module):
    def __init__(self, n_channels, out_channels, patch_height, patch_width):
        ...
        new_bands = math.ceil(n_channels / 32 - 1) * 32
        print("...new_bands *2  {}  ".format(new_bands*2))

        self.scale = 1
        self.inc = SingleConv(n_channels, 128)
        self.down1 = DownSingleConv(128, 256)
        # self.down2 = DownSingleConv(128 // self.scale, 256 // self.scale)
        self.down2 = DownSingleConv(256, out_channels)
        self.inc2 = SingleConv1(new_bands*4, out_channels)
        self.Conv1 = Conv(n_channels,200)
        self.Conv2 = Conv(200,300)
        self.Conv3 = Conv(300,out_channels)

        # self.down3 = DownSingleConv(256 // self.scale, out_channels)

    def forward(self, x):
        # x = self.inc(x)
        # # print("inc {}".format(x.shape))
        # x = self.down1(x)
        # # print("down1 {}".format(x.shape))
        #
        # x = self.down2(x)
        # print("down2 {}".format(x.shape))
