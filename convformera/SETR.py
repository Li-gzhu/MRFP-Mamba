...
class CNNAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., num_patches=1024):
        ...
        # self.dis = relative_pos_dis(math.sqrt(dim_head), math.sqrt(dim_head), sita=0.9).cuda()
    ...
    def forward(self, x, mode="train", smooth=1e-4):
        ...
        # print("dis 1   {}".format(dis.shape))

        dis = torch.exp(-dis)
        # print("dis 2   {}".format(dis.shape))

        dis = dis/torch.sum(dis, dim=-1)[:, :, None]
        # print("...dis 3  {}".format(dis.shape))
