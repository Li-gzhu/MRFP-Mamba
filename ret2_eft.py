...
class ret2(nn.Module):
    def __init__(self, patch_size, channels=103, num_classes=9, num_stages=2,
        ...
        # self.FasterNetBlock2_1 = FasterNetBlock2_1(hidden_dim[0],hidden_dim[0])
        # self.FasterNetBlock2_2 = FasterNetBlock2_2(hidden_dim[0],hidden_dim[1])
        self.convList = [self.Faster1, self.Faster2]
        self.Transformer = nn.ModuleList([Transformer(
            layers[i],  hidden_dim[i], heads[i], s[i],)for i in range(num_stages)])
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, heads, s, dropout=0,mlp_dim = 3,):
        super().__init__()
        self.s = s
