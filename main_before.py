...
from models_before import get_model, train, test, save_model, applyPCA
import get_cls_map
import argparse
RandomState = 345
...
parser.add_argument(
    ...
    help="Dataset to use IndianPines Salinas GRSS  PaviaU HyRANK  PaviaC HyRANK KSC Salinas whulk XA."
...
if palette is None:
    ...
    for k, color in enumerate(sns.color_palette("hls", len(LABEL_VALUES) - 1)):
        palette[k + 1] = tuple(np.asarray(255 * np.array(color), dtype="uint8"))#原本来dtype="uint8
...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
        ...
        probabilities = test(model, img, hyperparams)#得到一个包含各个类别的概率数组probabilities
        prediction = np.argmax(probabilities, axis=-1)#取 probabilities 数组中的最大值所对应的索引，即预测的类别标签。; 数组的最后一个维度上找到最大值的索引。
