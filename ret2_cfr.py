...
class ret2(nn.Module):
    def __init__(self, patch_size, channels=103, num_classes=9, num_stages=2,
        ...
        self.FirstConv1 = FirstConv(channels,hidden_dim[0])
        self.FirstConv2 = FirstConv(hidden_dim[0],hidden_dim[1])
        self.convList = [self.FirstConv1, self.FirstConv2]
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i != self.num_stages - 1:
                x = x.reshape(B, s, s, -1).permute(0, 3, 1, 2).contiguous()#图片
                # print("....  i  {}".format(i))
...
class RetNet(nn.Module):
    ...
    def forward(self, X):
        ...
        for i in range(self.layers):
            ...
            X = self.norm(X)
            # print("......after retnet  x.shape{}".format(X.shape))
        return X, s
...
class FirstConv(nn.Module):
    def __init__(self, embed_dim, embed_dim1, act_layer=nn.ReLU ):
        ...
        #                 act_layer()
        #             )
        self.proj = nn.Conv2d(embed_dim, embed_dim1, kernel_size=3, stride=1,)
...
# def ret3(dataset, patch_size):
#     model = None
#     if dataset == 'sa':
#         model = ret3(n_bands=204, num_classes=16)
#     elif dataset == 'pu':
#         model = ret3(n_bands=103, num_classes=9)
#     elif dataset == 'whulk':
#         model = ret3(n_bands=270, num_classes=9)
#     elif dataset == 'hrl':
#         model = ret3(n_bands=176, num_classes=14)
#     return model
...
if __name__ == "__main__":
    ...
    # # t = torch.randn(size=(3, 1, 204, 7, 7))#原本
    # t = torch.randn(size=(64, 1, 103, 13, 13))
    #
    # # net = proposed(dataset='sa', patch_size=7)  #原本
    # net = proposed(dataset='sa', patch_size=13)
    #
