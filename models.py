...
from models_our.models import ARSNet
from MRFP_Mamba import MRFP_Mamba, ARSNet3
from vit_pytorch.DCTN import DCTN
from vit_pytorch.DCTN import EISA
# utils
import math
import os
import datetime
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        optimizer = optim.Adam(model.parameers(), lr=lr)
        ...
        patch_size = kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = SSFTTnet(num_classes=n_classes)#net
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault("batch_size", 100)
        kwargs.setdefault('epoch', 100)
        ...
        # default of  0.002 to  train the  network"
        lr = kwargs.setdefault('lr', 0.0001)
        model = UMMA(padding_val=0, input_dim=225, embd_dim=256, n_classes = n_classes,device_umma=device)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    #     kwargs.setdefault("batch_size", 100)
        ...
        kwargs.setdefault("patch_size",15)
        center_pixel = True
        model = gaht(img_size=15,#(img_size=patch_size)
        ...
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 100)
    elif name == "MRFP_Mamba":
        kwargs.setdefault("patch_size",15)
        center_pixel = True
        model = MRFP_Mamba(#(img_size=patch_size)
            num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)#indian0.003
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size",15)
        center_pixel = True
        model = ARSNet3(img_size=15,#(img_size=patch_size)
            n_groups=[4,4,4], num_classes=n_classes, in_chans=n_bands,
                          )
        lr = kwargs.setdefault("learning_rate", 0.001)#indian0.003
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        layers = [2, 2, 5, 3]
        model = DCTN(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=[320, 320, 512, 512], patch_size=3,
                     transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [984, 984, 256, 256]## for IN 400, for GRSS 256, for PU 168, for KSC 352 for XA 480 for Houston2013 288
        model = HiT(layers, img_size=11, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        ...
        patch_size = kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ARSNet(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault("learning_rate", 0.0006)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault("batch_size", 100)
        kwargs.setdefault("epoch", 100)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
...
class HybridEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=15):
        ...
        self.conv4 = nn.Sequential(
            ...
            nn.ReLU()
        ) ## 6016 for IN, 576 for Grss ##2912 for PU3,5248 for KSC,Houston2013 4224
...
def test(net, img, hyperparams):
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            data = data.to(device)
            e = 100
            out = net(data)   # forward_output, e, e
            output = out
            if isinstance(output, tuple):
                output = output[0]
...
if __name__ == '__main__':
    input_tensor = torch.randn(2, 1, 200, 15, 15)
    ...
    layers = [2, 2, 5, 3]
    # model = gaht(img_size=15,#(img_size=patch_size)
    #         n_groups=[2,2,2], depths=[1,1,1], num_classes=16, in_chans=200,
    #                       )#SSFTTnet(num_classes=16)
    # model =DCTN(layers, img_size=15, in_chans=200, num_classes=16, embed_dims=[440, 440, 512, 512],
    #              patch_size=3,
    #              transitions=transitions,
    #              segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA).cuda() #ViT(dim=128, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1, num_classes=16, channels=200,).cuda()#TESTEtAl(input_channels=200, n_classes=16, patch_size=15).cuda()
    #model = SSFTTnet(num_classes=15)#DCTN(layers, img_size=15, in_chans=144, num_classes=15, embed_dims=[320, 320, 512, 512],
    #              patch_size=3,
    #              transitions=transitions,
    #              segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA)
    # model = DCTN(num_classes=15)
    # model =  gaht(img_size=15,#(img_size=patch_size)
    #         n_groups=[2,2,2], depths=[1,1,1], num_classes=15, in_chans=144,
    #                       )#ViT(dim=128, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1, num_classes=16, channels=144,).cuda()
    model  = ARSNet2(num_classes=16, in_chans=200,).cuda() #ViT(dim=128, image_size=15, patch_size=3, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1, num_classes=16, channels=144,).cuda()
#

# 计算参数量
    summary_info = summary(model, input_size=(2, 1, 200, 15, 15))
